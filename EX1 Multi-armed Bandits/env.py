...
class BanditEnv:
    ...
    def step(self, action: int) -> Tuple[float, int]:
        ...
        reward = np.random.normal(self.means[action], 1, 1)

        # generates random sample from normal distribution(bell curve)
        # takes, loc = mean of normal distribution, std deviation, size of o/p array
        # hence returning array[0] as the size is 1 here.
        return reward[0]