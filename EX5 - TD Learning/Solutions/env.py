...
def register_env() -> None:
    ...
    """
    # TODO
    # Regular WindyGridWorld Environment
    register(id="WindyGridWorld-v0", entry_point="env:WindyGridWorldEnv")
    # WindyGridWorld Kings Environment
    ...
    # Stoch WindyGridWorld Kings Environment
...
class WindyGridWorldEnv(Env):
    def __init__(self):
        ...
        # Wind TODO define self.wind as either a dict (keys would be states) or multidimensional array (states
    ...
    def step(self, action: Action) -> Tuple[Tuple[int, int], float, bool, dict]:
        ...
        new_x_pos = self.agent_pos[0] + dx - self.wind[self.agent_pos]
        new_x_pos = max(new_x_pos, 0)
        new_x_pos = min(new_x_pos, self.rows - 1)
        x = new_x_pos

        new_y_pos = self.agent_pos[1] + dy
        new_y_pos = max(new_y_pos, 0)
        new_y_pos = min(new_y_pos, self.cols - 1)
        y = new_y_pos
        self.agent_pos = (x, y)
...
class WindyGridWorldKingsEnv(Env):
    def __init__(self):
        ...
        # Wind define self.wind as either a dict (keys would be states) or multidimensional array (states correspond
        # to indices)
        self.wind = np.zeros((self.rows, self.cols), dtype=int)
        self.wind[:, [3, 4, 5, 8]] = 1
        self.wind[:, [6, 7]] = 2
    ...
    def step(self, action: Action) -> Tuple[Tuple[int, int], float, bool, dict]:
        ...
        # print(dx, dy)
        # new position of an agent in a grid-based environment
